      ...
      <el-row>
        ...
        <div v-else>
          <el-form-item v-if="ruleForm.meishitupian" label="美食图片" prop="meishitupian">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.meishitupian.split(',')" :src="item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
                                                                                                                                                                                                                        <el-col :span="12">
...
export default {
  ...
  methods: {
	...
	this.ruleForm.meishixiangqing = this.ruleForm.meishixiangqing.replace(reg,'../../../ssmx6010/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
        ...
        if (valid) {
          this.$http({
            url: `jingdianmeishi/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.jingdianmeishiCrossAddOrUpdateFlag = false;
                  this.parent.search();
